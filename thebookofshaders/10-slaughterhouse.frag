...
void main() {
    ...
    float m_x = (0.75-(u_mouse.x/u_resolution.x))*0.5;
    float r = step(m_x,random(ipos));
    
    y = step(mod(floor(-u_time*4.+1.),p.y+1.),m_st.y);
    
    float step_x = mod(u_time*p.y*8.,p.x);
    float x = 1.-step(step_x,m_st.x);
    
    float v = step(mod(floor(-u_time*4.),p.y+1.),m_st.y)-step(mod(floor(-u_time*4.+1.),p.y+1.),m_st.y);
    float m = v*x+y;
